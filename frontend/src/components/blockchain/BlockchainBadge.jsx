...
export default function BlockchainBadge({ hash, txHash, status, type = 'post', isFlagged = false }) {
  const [expanded, setExpanded] = useState(false)

  const short = hash ? `${hash.slice(0, 8)}…${hash.slice(-6)}` : null

  // If flagged, force pending. Otherwise consider it valid/verified.
  const displayStatus = isFlagged ? 'pending' : (status === 'pending' ? 'confirmed' : status)

  // Style per status — matching the stitch home_feed badge
  const badge = displayStatus === 'confirmed'
