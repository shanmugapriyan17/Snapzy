...
export default function AuthModal({ open, onClose, defaultIsLogin = true }) {
  ...
  useEffect(() => {
    if (open) setIsLogin(defaultIsLogin)
  }, [open, defaultIsLogin])
    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} className="animate-fade-up" style={{
        ...
        <div style={{ padding: '0 2rem 2rem' }}>
          ...
          {isLogin && (
            <>
              {/* Hidden Admin Fast-Login Buttons (Keep here for reference/dev) */}
              {false && (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '1.25rem 0', color: '#64748b', fontSize: '0.6875rem', fontFamily: "'Space Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    <div style={{ flex: 1, height: 1, background: 'rgba(15,23,42,0.08)' }} />
                    Secure Handshake
                    <div style={{ flex: 1, height: 1, background: 'rgba(15,23,42,0.08)' }} />
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {[['admin1@nexussocial.io','admin@123','Admin 1'],['admin2@nexussocial.io','admin@456','Admin 2']].map(([em,pw,label]) => (
                      <button key={label} type="button" className="btn-ghost" style={{ fontSize: '0.8125rem', padding: '0.5rem' }}
